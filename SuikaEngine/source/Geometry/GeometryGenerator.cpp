...
GeometryGenerator::MeshData GeometryGenerator::CreateCube()
{
	...
	int k = 0;
	for (int i = 0; i < sizeof(indices) / sizeof(uint32); i++)
	{
		Vertex vertex;
		vertex.Position = pos[indices[i]];
		vertex.Normal = normal[i / 3];
		meshData.Vertices.push_back(vertex);
		meshData.Indices32.push_back(k++);
	}
...
GeometryGenerator::MeshData GeometryGenerator::CreateScreenQuad()
{
	...
	meshData.Vertices[0].Position = XMFLOAT3(-1, -1, 0);	meshData.Vertices[0].TexC = XMFLOAT2(0, 0);
	meshData.Vertices[1].Position = XMFLOAT3(1, -1, 0);		meshData.Vertices[1].TexC = XMFLOAT2(1, 0);
	meshData.Vertices[2].Position = XMFLOAT3(-1, 1, 0);		meshData.Vertices[2].TexC = XMFLOAT2(0, 1);
	meshData.Vertices[3].Position = XMFLOAT3(1, 1, 0);		meshData.Vertices[3].TexC = XMFLOAT2(1, 1);
