...
public:
    ...
    __host__ __device__ DevCamera(float3 lookfrom, float3 lookat, float3 vup, float vfov, float aspect, float aperture, float focus_dist)
    {
        ...
        v = cross(w,u);
        // w = normalize(make_float3(0,0,1));
        // u = normalize(make_float3(1,0,0));
        // v = normalize(make_float3(0,1,0));
...
{
public:
    __device__ virtual bool scatter(const Ray& r_in, const HitRecord &rec, float3& attenuation, float3& emission, Ray& scattered, RandInstance& seed) const = 0;
};
