...
BVHNode* CreateBVH(CuVertex* g_vertices, unsigned& g_verticesNo,
	...
	CacheFriendlyBVHNode* g_pCFBVH)
{
	...
	if (!root->IsLeaf()) { // inner node
		...
		PopulateCacheFriendlyBVH(pFirstTriangle, p->_left, idxBoxes, idxTriList, g_triIndexList, g_pCFBVH);
		int idxRight = ++idxBoxes;
		PopulateCacheFriendlyBVH(pFirstTriangle, p->_right, idxBoxes, idxTriList, g_triIndexList, g_pCFBVH);
...
void CreateCFBVH(BVHNode* g_pSceneBVH, unsigned& g_triIndexListNo, int** g_triIndexList, 
	unsigned& g_pCFBVH_No, CacheFriendlyBVHNode** g_pCFBVH, Triangle* g_triangles)
{
	if (!g_pSceneBVH) {
	...
	*g_pCFBVH = new CacheFriendlyBVHNode[g_pCFBVH_No]; // array
	...
	PopulateCacheFriendlyBVH(&g_triangles[0], g_pSceneBVH, idxBoxes, idxTriList, *g_triIndexList, *g_pCFBVH);
...
void UpdateBoundingVolumeHierarchy(const char* filename, BVHNode** g_pSceneBVH, 
	unsigned& g_triIndexListNo, int** h_TriIdxList, unsigned& g_pCFBVH_No, CacheFriendlyBVHNode** g_pCFBVH,
	CuVertex* g_vertices, unsigned& g_verticesNo,
	Triangle* g_triangles, unsigned& g_trianglesNo)
{
	if (!(*g_pSceneBVH)) {
		std::string BVHcacheFilename = "../Resources/Cache/BVH/" + std::string(filename);
		BVHcacheFilename += ".bvh";
		FILE* fp = fopen(BVHcacheFilename.c_str(), "rb");
		if (!fp) {
			// No cached BVH data - we need to calculate them
			Clock me;
			*g_pSceneBVH = CreateBVH(g_vertices, g_verticesNo, g_triangles, g_trianglesNo);
			...
			CreateCFBVH(*g_pSceneBVH, g_triIndexListNo, h_TriIdxList,
				g_pCFBVH_No, g_pCFBVH, g_triangles);
			...
			if (g_triIndexListNo != fwrite(*h_TriIdxList, sizeof(int), g_triIndexListNo, fp)) return;
			fclose(fp);
		}
			...
			*h_TriIdxList = new int[g_triIndexListNo];
			if (g_pCFBVH_No != fread(*g_pCFBVH, sizeof(CacheFriendlyBVHNode), g_pCFBVH_No, fp)) return;
			if (g_triIndexListNo != fread(*h_TriIdxList, sizeof(int), g_triIndexListNo, fp)) return;
			fclose(fp);
		}
	}
}
