...
public:
	...
	{
		m_qd3dWidget = qd3d;
		m_d3dDevice = qd3d->m_d3dDevice.Get();
		m_CommandList = qd3d->m_CommandList.Get();
	}
	...
	std::unique_ptr<Texture> CreateTexture(std::string name, std::wstring filepath)
	{
		...
		else if (postfix == L"bmp")
		{
			...
			//void* ptr = nullptr;
			//Tex->UploadHeap->Map(0, nullptr, &ptr);
			//memcpy(reinterpret_cast<unsigned char*>(ptr) + footprint.Offset, image.pixels.data(), pixel_data_size);
			...
			//src.pResource = Tex->UploadHeap.Get();
